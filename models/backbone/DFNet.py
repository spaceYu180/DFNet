...
default_cfgs = {
    'dfnet_small': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_small.pth'),
    'dfnet_base': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_base.pth'),
    'dfnet_large': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_large.pth'),
    'dfnet_small_384': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_small_384.pth',
                              input_size=(3, 384, 384), crop_pct=1.0),
    'dfnet_base_384': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_base_384.pth',
                             input_size=(3, 384, 384), crop_pct=1.0),
    'dfnet_large_384': _cfg(url='https://huggingface.co/sail/dl2/resolve/main/iformer/iformer_large_384.pth',
                              input_size=(3, 384, 384), crop_pct=1.0),
}
...
@register_model
def dfnet_small(pretrained=False, **kwargs):
    """
    ...
    model = DFNet(img_size=224,
                  ...
                  **kwargs)
    model.default_cfg = default_cfgs['dfnet_small']
    if pretrained:
...
@register_model
def dfnet_small_384(pretrained=False, **kwargs):
    depths = [3, 3, 9, 3]
    embed_dims = [96, 192, 320, 384]
    num_heads = [3, 6, 10, 12]
    attention_heads = [1] * 3 + [3] * 3 + [7] * 4 + [9] * 5 + [11] * 3

    model = DFNet(img_size=384,
                  depths=depths,
                  embed_dims=embed_dims,
                  num_heads=num_heads,
                  attention_heads=attention_heads,
                  use_layer_scale=True, layer_scale_init_value=1e-6,
                  **kwargs)
    model.default_cfg = default_cfgs['dfnet_small_384']
    if pretrained:
        url = model.default_cfg['url']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        model.load_state_dict(checkpoint)
...
@register_model
def dfnet_base(pretrained=False, **kwargs):
    """
    ...
    model = DFNet(img_size=224,
                  ...
                  use_layer_scale=True, layer_scale_init_value=1e-6,
                  **kwargs)
    model.default_cfg = default_cfgs['dfnet_base']
    if pretrained:
        url = model.default_cfg['url']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        model.load_state_dict(checkpoint)
...
@register_model
def dfnet_base_384(pretrained=False, **kwargs):
    depths = [4, 6, 14, 6]
    embed_dims = [96, 192, 384, 512]
    num_heads = [3, 6, 12, 16]
    attention_heads = [1] * 4 + [3] * 6 + [8] * 7 + [10] * 7 + [15] * 6
    ...
    model = DFNet(img_size=384,
                  ...
                  use_layer_scale=True, layer_scale_init_value=1e-6,
                  **kwargs)
    model.default_cfg = default_cfgs['dfnet_base_384']
    if pretrained:
        url = model.default_cfg['url']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        model.load_state_dict(checkpoint)
...
@register_model
def dfnet_large(pretrained=False, **kwargs):
    """
    ...
    model = DFNet(img_size=224,
                                 ...
                                 use_layer_scale=True, layer_scale_init_value=1e-6,
                                 **kwargs)
    model.default_cfg = default_cfgs['dfnet_large']
    if pretrained:
        url = model.default_cfg['url']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
...
@register_model
def dfnet_large_384(pretrained=False, **kwargs):
    depths = [4, 6, 18, 8]
    embed_dims = [96, 192, 448, 640]
    num_heads = [3, 6, 14, 20]
    attention_heads = [1] * 4 + [3] * 6 + [10] * 9 + [12] * 9 + [19] * 8
    ...
    model = DFNet(img_size=384,
                  ...
                  use_layer_scale=True, layer_scale_init_value=1e-6,
                  **kwargs)
    model.default_cfg = default_cfgs['dfnet_large_384']
    if pretrained:
        url = model.default_cfg['url']
        checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True)
        model.load_state_dict(checkpoint)
...
if __name__ == "__main__":
    ii = torch.rand(4,3,256,256)
    models = dfnet_large(pretrained=True)
    oo = models(ii)
